...
def get_links(driver, projects_temp):
    ...
    for i in range(2, 51):
        ...
        for item in items:
            privacy = item.find_element(By.TAG_NAME, "button").get_attribute("title")
            if "Public" in privacy:
                a = item.find_element(By.TAG_NAME, "a")
                link = a.get_attribute("href")
                projects_temp.append(link)
            else:
                non_public_projects_counter += 1

        driver.get(f"https://git.engee.com/explore?page={i}")

    stop = perf_counter()
    print(f"USED TIME TO GET ALL LINKS: {stop - start}")
    print(f"FIND NOT A PUBLIC PROJECTS: {non_public_projects_counter}")
...
if __name__ == "__main__":
    ...
    print(f"PROJECT WITH ISSUES ({len(issues_projects)}): {issues_projects}")


    with open("../links_with_dublicate_filter.txt", mode="w", encoding="UTF-8") as output:
